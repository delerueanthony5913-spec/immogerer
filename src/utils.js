...
export const isSundayOrHoliday = (dateStr) => {
  ...
  const asc = new Date(paques); asc.setDate(paques.getDate() + 39);
  const lPen = new Date(paques); lPen.setDate(paques.getDate() + 50);
  holidays.push(formatLocal(lP), formatLocal(asc), formatLocal(lPen));
