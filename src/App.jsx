...
// NOUVEAU GRAPHIQUE MULTI-COURBES DYNAMIQUE
const ComparisonChart = ({ data, properties, platforms, yearsAvailable }) => {
  ...
  useEffect(() => {
    if (mode === 'years') setSelectedKeys(yearsAvailable.slice(0, 3)); // Sélectionne les 3 dernières années par défaut
    if (mode === 'properties') setSelectedKeys(properties.map(p => p.id).slice(0, 4)); // Max 4 logements par défaut
    if (mode === 'platforms') setSelectedKeys(platforms.slice(0, 4)); // Max 4 plateformes par défaut
  ...
  const series = selectedKeys.map((key, index) => {
      ...
      if (!isPastCurrentYear) {
         if (mode === 'years' && parseInt(key) > parseInt(currentYear)) splitIndex = -1; // Tout futur
         else if (mode === 'years' && parseInt(key) === parseInt(currentYear)) splitIndex = currentMonth; // Coupe ce mois ci
  ...
  const allDataValues = series.map(s => s.data).flat();
