...
import { 
  ...
  Filter, Loader2, CalendarRange, Mail, Link, ArrowRight, LocateFixed, TrendingUp, TrendingDown, Key, UploadCloud, Copy
} from 'lucide-react';
  ...
  const buildSeriesFor = (targetYear, targetProp, targetPlat) => {
      ...
      safeData.forEach(t => {
          ...
          if (metric === 'net' || metric === 'gross') {
              if (isDirect) {
                  const a1 = parseFloat(t.acompte1Amount) || 0; const s = parseFloat(t.soldeAmount) || 0;
                  if (t.acompte1Date) processItem(t.acompte1Date, a1, false);
                  if (t.soldeDate) { processItem(t.soldeDate, s, false); if (metric === 'net' && tax > 0) processItem(t.soldeDate, -tax, false); }
              } else {
      ...
      return { data: res.map(val => isNaN(val) ? 0 : val) };
  };

  const series = selectedKeys.map((key, index) => {
      const result = buildSeriesFor(key, 'all', 'all');
      return { id: key, data: result.data, color: CHART_COLORS[index % CHART_COLORS.length], label: key };
  });

  const w = 900, h = 250, padX = 60, padY = 30; 
  const getY = (val) => h - padY - ((val / Math.max(...series.flatMap(s=>s.data), 100)) * (h - 2 * padY));
  ...
  return (
    <div className="bg-white p-6 rounded-[32px] shadow-xl border border-slate-50 mt-8 overflow-hidden">
      <div className="flex gap-2 mb-6">
        <button onClick={()=>setMode('years')} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase ${mode==='years'?'bg-slate-900 text-white':'bg-slate-100 text-slate-400'}`}>Années</button>
      </div>
      <div className="overflow-x-auto hide-scroll">
        <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} className="overflow-visible">
          {series.map(s => (
            <path key={s.id} d={`M ${s.data.map((v, i) => `${getX(i)},${getY(v)}`).join(' L ')}`} fill="none" stroke={s.color} strokeWidth="3" />
          ))}
        </svg>
      </div>
    </div>
...
const App = () => {
  ...
  const generateICalLInk = (propertyId) => `${window.location.origin}${window.location.pathname}?ical=${propertyId}`;
  ...
  const detailedExpenses = useMemo(() => {
    const list = []; baseTenants.forEach(t => (t.resExpenses || []).forEach(exp => { if ((filterProv === 'all' || exp.person === filterProv) && checkDateFilter(exp.paymentDate || t.startDate)) list.push({ id: `${t.id}-${exp.id}`, propertyName: properties.find(p => p.id === t.propertyId)?.name || '--', dateRes: t.startDate, person: exp.person, amount: parseFloat(exp.amount) || 0, paymentDate: exp.paymentDate || '' }); }));
