...
// --- 1. FONCTIONS UTILITAIRES (HORS COMPOSANT POUR ACCÈS GLOBAL) ---
...
const App = () => {
  ...
  const reservationsList = useMemo(() => filteredData.sort((a, b) => (a.startDate || "").localeCompare(b.startDate || "")), [filteredData]);
        ...
        {quickPayConfig && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
            <div className="bg-white p-8 rounded-[40px] shadow-2xl max-w-sm w-full border border-slate-100 flex flex-col gap-6 animate-in zoom-in-95 text-center">
              <h3 className="font-black text-xl uppercase">Valider paiement</h3>
              ...
              <div className="flex gap-3"><button onClick={() => setQuickPayConfig(null)} className="flex-1 p-4 rounded-2xl text-slate-400">Annuler</button><button onClick={async () => {
              ...
              }} className="flex-1 p-4 rounded-2xl bg-emerald-500 text-white font-black">Encaisser</button></div>
            </div>
          </div>
        )}
