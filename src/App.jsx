    ...
    <div className="bg-white p-4 md:p-10 rounded-[24px] md:rounded-[48px] border border-gray-50 flex flex-col md:flex-row items-center gap-4 md:gap-10 shadow-xl shadow-slate-200/50 mx-2 md:mx-0">
      ...
      <div className="flex-1 w-full space-y-1.5 md:space-y-3">
        ...
        <div className="space-y-1 md:space-y-2">
          {visibleData.map((slice, i) => (
            <div key={i} className="flex items-center justify-between text-[9px] md:text-[11px]">
...
const App = () => {
  ...
  const saveRes = async (e) => {
    e.preventDefault(); if (!formData.propertyId || !formData.name || !formData.startDate || !formData.endDate) { alert("Champs obligatoires manquants."); return; }
    ...
    const d = { ...formData, isUrssaf: formData.isUrssaf !== false, grossAmount: g, netAmount: n, platformFees: plat, bankFees: bank, cityTax: city, displayedAmount: disp, acompte1Amount: a1, acompte2Amount: a2, soldeAmount: s, resExpenses: (formData.resExpenses || []).map(r => ({ ...r, amount: parseFloat(r.amount) || 0, hoursEntry: parseFloat(r.hoursEntry) || 0, rateEntry: parseFloat(r.rateEntry) || 0, hoursExit: parseFloat(r.hoursExit) || 0, rateExit: parseFloat(r.rateExit) || 0 })) };
  ...
  const handleQuickPayToggle = async (e, tenant, type, expId = null) => {
    ...
    if (type === 'global' && tenant.platform === 'En direct') { setEditingResId(tenant.id); setFormData(tenant); setIsModalOpen(true); return; }
    let isPaid = type === 'global' ? !!tenant.paymentDate : !!(tenant.resExpenses || []).find(x => x.id === expId)?.paymentDate;
    if (isPaid) { if (window.confirm("Annuler paiement ?")) { if (type === 'global') { await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'tenants', tenant.id), { paymentDate: '' }, { merge: true }); } else { const newExpenses = tenant.resExpenses.map(exp => exp.id === expId ? { ...exp, paymentDate: '' } : exp); await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'tenants', tenant.id), { resExpenses: newExpenses }, { merge: true }); } } }
  ...
  const submitQuickPay = async () => {
    ...
    else { const newExpenses = quickPayConfig.tenant.resExpenses.map(exp => exp.id === quickPayConfig.expId ? { ...exp, paymentDate: quickPayConfig.date } : exp); await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'tenants', quickPayConfig.tenant.id), { resExpenses: newExpenses }, { merge: true }); }
    setQuickPayConfig(null);
  };

  const onTouchStart = (e) => { if (e.touches && e.touches.length > 1) return; if (e.target.closest('.no-swipe')) return; };
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col md:flex-row font-sans text-slate-900">
      ...
      <main className="flex-1 w-full min-w-0 min-h-screen relative flex flex-col overflow-x-hidden">
        {quickPayConfig && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
            <div className="bg-white p-8 rounded-[40px] shadow-2xl max-w-sm w-full border border-slate-100 flex flex-col gap-6 animate-in zoom-in-95 text-center">
              ...
              <input type="date" value={quickPayConfig.date} onChange={e => setQuickPayConfig({...quickPayConfig, date: e.target.value})} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl font-black text-center text-lg outline-none" />
              <div className="flex gap-3">
                <button onClick={() => setQuickPayConfig(null)} className="flex-1 p-4 rounded-2xl font-black uppercase text-[10px] text-slate-400 bg-slate-50">Annuler</button>
                <button onClick={submitQuickPay} className="flex-1 p-4 rounded-2xl font-black uppercase text-[10px] text-white bg-emerald-500 shadow-xl shadow-emerald-200">Encaisser</button>
              </div>
            </div>
          </div>
        )}
        ...
        <div ref={scrollContainerRef} onScroll={handleScroll} className="flex-1 w-full flex overflow-x-auto snap-x snap-mandatory hide-scroll">
          <div className="flex-none w-full max-w-full snap-center snap-always px-0 md:px-12 py-6 md:py-12 box-border" style={{ scrollSnapStop: 'always' }}><div className="max-w-7xl mx-auto pb-32"><div className="flex justify-between items-center mx-2 md:mx-0 mb-6"><h2 className="text-2xl md:text-3xl font-black uppercase tracking-tighter">Réservations</h2><div className="flex items-center gap-2 md:gap-4"><button onClick={() => scrollToCurrentRes(true)} className="p-3 md:px-4 md:py-3 bg-white text-blue-600 rounded-full md:rounded-[20px] shadow-lg border border-slate-100 hover:bg-blue-50 transition-all flex items-center justify-center gap-2"><LocateFixed size={18} /><span className="hidden md:inline font-black text-[10px] uppercase">Aujourd'hui</span></button><button onClick={() => { setEditingResId(null); setFormData({ propertyId: properties[0]?.id || '', name: '', phone: '', startDate: '', endDate: '', paymentDate: '', platform: availablePlatforms[0] || 'Airbnb', isUrssaf: true, displayedAmount: '', cityTax: '', bankFees: '', grossAmount: '', platformFees: '', deposit: '', resExpenses: [], comment: '', acompte1Amount: '', acompte1Date: '', acompte2Amount: '', acompte2Date: '', soldeAmount: '', soldeDate: '' }); setIsModalOpen(true); }} className="bg-blue-600 text-white px-6 py-3 md:px-8 md:py-4 rounded-[20px] md:rounded-[24px] font-black text-[11px] shadow-xl hover:bg-blue-700 transition-all">+ Nouvelle</button></div></div><div className="md:hidden max-h-[70vh] overflow-y-auto custom-scrollbar p-1 rounded-[20px] border border-slate-100 bg-slate-50/50 shadow-inner mx-2 relative"><div className="grid grid-cols-1 gap-2.5">{(groupedReservationsList || []).map(item => { if (item.isSeparator) return <div key={item.id} className="flex items-center justify-center mt-2 mb-0.5"><span className="bg-slate-800 text-white px-4 py-1.5 rounded-[10px] text-[8px] font-black uppercase tracking-[0.2em] shadow-sm">{item.label}</span></div>; const t = item; const c = getRowColors(t.propertyId); return <div key={t.id} data-res-id={t.id} onClick={() => { setEditingResId(t.id); setFormData(t); setIsModalOpen(true); }} className={`${c.bg} p-3 rounded-[16px] shadow-sm border border-slate-50 cursor-pointer transition-colors`}><div className="flex justify-between items-start mb-1.5"><div><h3 className="text-sm font-black uppercase leading-tight">{(properties || []).find(p => p.id === t.propertyId)?.name || '--'}</h3><div className="flex items-center gap-1.5 mt-1 leading-tight"><span className="text-[10px] font-black text-blue-600 uppercase tracking-widest">{t.platform}</span><span className="text-[11px] font-black text-slate-700">{t.name}</span></div></div><div className="flex flex-col items-end"><span onClick={(e) => handleQuickPayToggle(e, t, 'global')} className={`px-2 py-0.5 rounded-full text-[7px] font-black uppercase cursor-pointer transition-transform inline-block ${getStatusProps(t).color}`}>{getStatusProps(t).label}</span>{t.paymentDate && t.platform !== 'En direct' && <span className="text-[7px] text-slate-400 mt-1 font-bold">{formatDateFr(t.paymentDate)}</span>}{t.platform === 'En direct' && t.soldeDate && <span className="text-[7px] text-slate-400 mt-1 font-bold">{formatDateFr(t.soldeDate)}</span>}</div></div><div className="bg-white/60 p-2 rounded-[12px] flex justify-between font-black text-[9px] mb-1.5 items-center"><span>{formatDateFr(t.startDate)}</span><ArrowRight size={10} className="text-slate-300"/><span>{formatDateFr(t.endDate)}</span></div>{t.comment && <div className="text-[9px] italic text-slate-600 mb-1.5 px-1 leading-tight whitespace-pre-wrap">📝 {t.comment}</div>}{t.resExpenses && t.resExpenses.length > 0 && <div className="space-y-1 border-t border-slate-100 pt-1.5 mb-1.5">{(t.resExpenses || []).map((exp, idx) => (<div key={idx} onClick={(e) => handleQuickPayToggle(e, t, 'expense', exp.id)} className="flex items-center justify-between text-[8px] bg-white/60 p-1.5 rounded-xl cursor-pointer transition-colors leading-tight"><span className="uppercase font-black text-slate-500 min-w-0 truncate pr-2">{exp.type} ({exp.person}) {exp.sendEmail !== false && providerEmails[exp.person] && !exp.person.toLowerCase().includes('dias') && <Mail size={8} className="inline text-blue-500 flex-shrink-0"/>}</span><div className="text-right flex-shrink-0"><span className={`font-black flex items-center justify-end gap-1 ${exp.paymentDate ? 'text-emerald-600' : 'text-orange-500'}`}>{exp.amount}€ {exp.paymentDate ? <CheckCircle size={8}/> : <Clock size={8}/>}</span>{exp.paymentDate && <div className="text-[7px] text-slate-400 mt-0.5">{formatDateFr(exp.paymentDate)}</div>}</div></div>))}</div>}<div className="text-right font-black text-sm">{(parseFloat(t.netAmount) || 0).toFixed(2)}€</div></div>; })}</div></div><div className="hidden md:block bg-white rounded-[40px] shadow-2xl overflow-hidden border border-slate-100"><div className="max-h-[70vh] overflow-y-auto custom-scrollbar relative"><table className="w-full text-left text-xs"><thead className="bg-slate-50 font-black uppercase border-b text-slate-400 sticky top-0 z-20 shadow-sm"><tr><th className="p-4 w-[15%]">Logement</th><th className="p-4 w-[15%]">Client</th><th className="p-4 w-[12%] text-center">Dates</th><th className="p-4 w-[25%]">Notes</th><th className="p-4 w-[18%]">Prestations</th><th className="p-4 text-right">Net</th><th className="p-4 text-center">État</th></tr></thead><tbody className="divide-y divide-slate-50 font-bold">{(groupedReservationsList || []).map(item => { if (item.isSeparator) return <tr key={item.id} className="bg-slate-100/50"><td colSpan="7" className="p-3 text-center"><span className="bg-slate-800 text-white px-5 py-2 rounded-[14px] text-[10px] font-black uppercase tracking-[0.2em] shadow-md inline-block">{item.label}</span></td></tr>; const t = item; const c = getRowColors(t.propertyId); return <tr key={t.id} data-res-id={t.id} onClick={() => { setEditingResId(t.id); setFormData(t); setIsModalOpen(true); }} className={`${c.bg} cursor-pointer transition-colors hover:bg-slate-50`}><td className="p-4 uppercase"><div className="font-black">{(properties || []).find(p => p.id === t.propertyId)?.name || '--'}</div><div className="text-blue-600 text-xs font-black tracking-widest mt-0.5">{t.platform}</div></td><td className="p-4"><div className="text-sm font-black">{t.name}</div>{t.phone && <div className="text-slate-400 text-[10px] mt-0.5">{t.phone}</div>}</td><td className="p-4 text-center text-slate-500 whitespace-nowrap">{formatDateFr(t.startDate)} <ArrowRight size={10} className="inline text-slate-300" /> {formatDateFr(t.endDate)}</td><td className="p-4 text-[11px] text-slate-600 font-medium">{t.comment ? <div className="bg-slate-50/50 p-2 rounded-xl border border-slate-100/50 italic whitespace-pre-wrap">📝 {t.comment}</div> : ''}</td><td className="p-4"><div className="space-y-1.5">{(t.resExpenses || []).map((exp, idx) => (<div key={idx} onClick={(e) => handleQuickPayToggle(e, t, 'expense', exp.id)} className="flex items-center justify-between text-[10px] bg-white/50 p-1.5 rounded-lg border border-slate-100/50 cursor-pointer hover:border-blue-300 hover:bg-white hover:shadow-sm transition-all"><span className="uppercase font-black text-slate-500 leading-none">{exp.type} ({exp.person}) {exp.sendEmail !== false && providerEmails[exp.person] && !exp.person.toLowerCase().includes('dias') && <Mail size={10} className="inline text-blue-500 ml-1"/>}</span><div className="text-right"><div className="flex items-center justify-end gap-1.5"><span className={`font-black ${exp.paymentDate ? 'text-emerald-600' : 'text-orange-500'}`}>{exp.amount}€</span>{exp.paymentDate ? <CheckCircle size={10} className="text-emerald-500" /> : <Clock size={10} className="text-orange-400" />}</div>{exp.paymentDate && <div className="text-[8px] text-slate-400 mt-0.5 leading-none">{formatDateFr(exp.paymentDate)}</div>}</div></div>))}</div></td><td className="p-4 text-right font-black">{(parseFloat(t.netAmount) || 0).toFixed(2)}€</td><td className="p-4 text-center"><div className="flex flex-col items-center"><span onClick={(e) => handleQuickPayToggle(e, t, 'global')} className={`px-4 py-2 rounded-full text-[9px] uppercase cursor-pointer hover:scale-105 transition-transform inline-block ${getStatusProps(t).color}`}>{getStatusProps(t).label}</span>{t.platform !== 'En direct' && t.paymentDate && <span className="text-[8px] text-slate-400 mt-1 font-bold">{formatDateFr(t.paymentDate)}</span>}{t.platform === 'En direct' && t.soldeDate && <span className="text-[8px] text-slate-400 mt-1 font-bold">{formatDateFr(t.soldeDate)}</span>}</div></td></tr>; })}</tbody></table></div></div></div></div>
          <div className="flex-none w-full max-w-full snap-center snap-always px-0 md:px-12 py-6 md:py-12 box-border" style={{ scrollSnapStop: 'always' }}><div className="max-w-7xl mx-auto pb-32"><div className="flex justify-between items-center mx-2 md:mx-0 mb-6"><div><h2 className="text-2xl md:text-3xl font-black uppercase">Agenda</h2></div><div className="flex items-center gap-4 bg-white px-4 py-2 rounded-2xl shadow-lg"><button onClick={()=>handleMonthChange('prev')}><ChevronLeft/></button><div className="text-center font-black min-w-[120px] uppercase text-xs">{['Janvier','Février','Mars','Avril','Mai','Juin','Juillet','Août','Septembre','Octobre','Novembre','Décembre'][filterMonth==='all'?new Date().getMonth():parseInt(filterMonth)]}</div><button onClick={()=>handleMonthChange('next')}><ChevronRight/></button></div></div><div className="bg-white p-4 md:p-6 rounded-[32px] md:rounded-[40px] shadow-2xl overflow-x-auto mx-2 md:mx-0"><div className="min-w-[320px] md:min-w-[700px]"><div className="grid grid-cols-7 text-center font-black text-slate-300 text-[8px] md:text-[10px] uppercase mb-2 md:mb-4">{['Lun','Mar','Mer','Jeu','Ven','Sam','Dim'].map(d=><div key={d}>{d}</div>)}</div><div className="grid grid-cols-7 gap-1 md:gap-2">{(agendaDays || []).map((item,idx)=>{ if(item.empty) return <div key={idx} className="h-16 md:h-32 bg-slate-50/30 rounded-xl md:rounded-2xl"></div>; const dayRes = (reservationsList || []).filter(r=>item.dateStr>=r.startDate && item.dateStr<=r.endDate); return (<div key={item.dateStr} className={`h-16 md:h-32 border rounded-xl md:rounded-2xl p-1 md:p-2 relative flex flex-col ${item.dateStr===todayStr?'border-blue-500 bg-blue-50/10':'border-slate-100'}`}><span className="text-[8px] md:text-[10px] font-black text-slate-300">{item.day}</span><div className="flex-1 space-y-0.5 md:space-y-1 overflow-y-auto no-scrollbar">{dayRes.map(r=>(<div key={r.id} onClick={(e)=>{e.stopPropagation();setEditingResId(r.id);setFormData(r);setIsModalOpen(true)}} className="text-[6px] md:text-[8px] font-black text-white p-0.5 md:p-1 rounded truncate cursor-pointer leading-tight" style={{backgroundColor: CHART_COLORS[(properties || []).findIndex(p=>p.id===r.propertyId)%CHART_COLORS.length]}}>{r.name?.split(' ')[0] || 'Résa'}</div>))}</div></div>);})}</div></div></div></div></div>
          <div className="flex-none w-full max-w-full snap-center snap-always px-0 md:px-12 py-6 md:py-12 box-border" style={{ scrollSnapStop: 'always' }}><div className="max-w-7xl mx-auto pb-32 space-y-10"><div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mx-2 md:mx-0"><div><h2 className="text-3xl md:text-4xl font-black uppercase text-slate-900 tracking-tighter leading-none mb-2">Statistiques</h2><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Tableau de bord dynamique</p></div></div><div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 mx-2 md:mx-0"><div className="bg-white p-3 md:p-6 rounded-[20px] md:rounded-[32px] shadow-xl border border-slate-50 flex flex-col justify-center items-center text-center h-24 md:h-40 relative overflow-hidden group hover:scale-[1.03] transition-transform"><p className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 md:mb-2 z-10">CA Brut ({statsCalculations.year})</p><p className="text-xl md:text-3xl font-black text-indigo-600 z-10">{Math.round(statsCalculations.currentYearGross).toLocaleString('fr-FR')}€</p><div className={`mt-1 md:mt-2 flex items-center gap-1 text-[8px] md:text-[10px] font-black px-2 py-0.5 md:py-1 rounded-full z-10 ${statsCalculations.grossGrowth >= 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>{statsCalculations.grossGrowth >= 0 ? <TrendingUp size={10}/> : <TrendingDown size={10}/>} {statsCalculations.grossGrowth}% vs {statsCalculations.prevYear}</div></div><div className="bg-white p-3 md:p-6 rounded-[20px] md:rounded-[32px] shadow-xl border border-slate-50 flex flex-col justify-center items-center text-center h-24 md:h-40 relative group hover:scale-[1.03] transition-transform"><p className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 md:mb-2">CA à venir (Impayés)</p><p className="text-xl md:text-3xl font-black text-blue-600">{Math.round(statsCalculations.upcomingGross).toLocaleString('fr-FR')}€</p></div><div className="bg-white p-3 md:p-6 rounded-[20px] md:rounded-[32px] shadow-xl border border-slate-50 flex flex-col justify-center items-center text-center h-24 md:h-40 group hover:scale-[1.03] transition-transform"><p className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 md:mb-2">Coût Prestations</p><p className="text-xl md:text-3xl font-black text-rose-500">-{Math.round(statsCalculations.currentYearExp).toLocaleString('fr-FR')}€</p></div><div className="bg-white p-3 md:p-6 rounded-[20px] md:rounded-[32px] shadow-xl border border-slate-50 flex flex-col justify-center items-center text-center h-24 md:h-40 group hover:scale-[1.03] transition-transform"><p className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 md:mb-2">CA / Nuit</p><p className="text-xl md:text-3xl font-black text-emerald-600">{statsCalculations.revPerNight}€</p></div></div><ComparisonChart data={baseTenants} properties={properties} platforms={availablePlatforms} yearsAvailable={yearsAvailable} /><div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-10 mx-2 md:mx-0"><DonutChart title={`Net (Banque) / Logement (${filterYear})`} data={(properties || []).map((p,idx)=>({label:p.name,value:(baseTenants || []).reduce((acc,t) => t.propertyId===p.id ? acc + getTenantProfitForStats(t) : acc, 0),color:CHART_COLORS[idx%CHART_COLORS.length]}))} /><DonutChart title={`Net (Banque) / Plateforme (${filterYear})`} data={(availablePlatforms || []).map((p,idx)=>({label:p,value:(baseTenants || []).reduce((acc,t) => t.platform===p ? acc + getTenantProfitForStats(t) : acc, 0),color:CHART_COLORS[(idx+4)%CHART_COLORS.length]}))} /></div></div></div>
          <div className="flex-none w-full max-w-full snap-center snap-always px-0 md:px-12 py-6 md:py-12 box-border" style={{ scrollSnapStop: 'always' }}><div className="max-w-7xl mx-auto pb-32 space-y-10"><h2 className="text-3xl font-black uppercase mx-2 md:mx-0">Comptabilité</h2><div className="bg-white rounded-[24px] md:rounded-[40px] shadow-2xl overflow-hidden text-xs border border-slate-100 mx-2 md:mx-0"><div className="p-3 md:p-8 bg-slate-900 text-white font-black uppercase flex justify-between items-center text-[10px] md:text-xs"><div>Bilan Global</div></div><div className="max-h-[60vh] overflow-y-auto overflow-x-auto custom-scrollbar relative"><table className="w-full text-left min-w-[280px] md:min-w-[700px]"><thead className="bg-slate-50 uppercase text-slate-400 border-b sticky top-0 z-10 shadow-sm text-[6px] md:text-xs tracking-tighter md:tracking-normal"><tr><th className="p-1 md:p-6">Période</th><th className="p-1 md:p-6 text-right">Brut URSSAF</th><th className="p-1 md:p-6 text-right text-emerald-600">Direct (hors)</th><th className="p-1 md:p-6 text-right text-indigo-600">Virement</th><th className="p-1 md:p-6 text-right text-slate-500">Prest.</th><th className="p-1 md:p-6 text-right text-rose-500">Cotis.</th><th className="p-1 md:p-6 text-right font-black">Profit</th></tr></thead><tbody className="divide-y font-bold">{(monthlyRecapData || []).map(([m, d]) => (<tr key={m} className="group hover:bg-slate-50/50 transition-colors"><td className="p-1 md:p-6 capitalize text-[8px] md:text-sm leading-tight tracking-tighter md:tracking-normal">{formatMonthYear(m)}</td><td className="p-1 md:p-6 text-right text-slate-500 text-[8px] md:text-sm">{d.urssafGross.toLocaleString('fr-FR')}€</td><td className="p-1 md:p-6 text-right text-emerald-600 font-black text-[8px] md:text-sm">{d.directNet > 0 ? `${d.directNet.toLocaleString('fr-FR')}€` : '-'}</td><td className="p-1 md:p-6 text-right text-indigo-600 font-black text-[8px] md:text-sm">{d.totalBank.toLocaleString('fr-FR')}€</td><td className="p-1 md:p-6 text-right text-slate-500 text-[8px] md:text-sm">-{d.charges.toLocaleString('fr-FR')}€</td><td className="p-1 md:p-6 text-right text-rose-500 text-[8px] md:text-sm">-{d.taxes.toFixed(2)}€</td><td className={`p-1 md:p-6 text-right font-black text-[8px] md:text-sm ${d.totalBank - d.taxes - d.charges >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{(d.totalBank - d.taxes - d.charges).toLocaleString('fr-FR')}€</td></tr>))}</tbody><tfoot className="bg-indigo-600 text-white font-black text-[10px] md:text-lg sticky bottom-0 z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]"><tr><td className="p-1.5 md:p-8 uppercase text-[6px] md:text-[10px]">TOTAL</td><td className="p-1.5 md:p-8 text-right opacity-90">{monthlyRecapData.reduce((acc, [m, d]) => acc + d.urssafGross, 0).toLocaleString('fr-FR')}€</td><td className="p-1.5 md:p-8 text-right text-emerald-300">{monthlyRecapData.reduce((acc, [m, d]) => acc + d.directNet, 0).toLocaleString('fr-FR')}€</td><td className="p-1.5 md:p-8 text-right">{monthlyRecapData.reduce((acc, [m, d]) => acc + d.totalBank, 0).toLocaleString('fr-FR')}€</td><td className="p-1.5 md:p-8 text-right text-indigo-200">-{monthlyRecapData.reduce((acc, [m, d]) => acc + d.charges, 0).toLocaleString('fr-FR')}€</td><td className="p-1.5 md:p-8 text-right text-rose-300">-{monthlyRecapData.reduce((acc, [m, d]) => acc + d.taxes, 0).toLocaleString('fr-FR')}€</td><td className="p-1.5 md:p-8 text-right bg-indigo-700/50">{(monthlyRecapData.reduce((acc, [m, d]) => acc + d.totalBank, 0) - monthlyRecapData.reduce((acc, [m, d]) => acc + d.taxes + d.charges, 0)).toLocaleString('fr-FR')}€</td></tr></tfoot></table></div></div><div className="bg-white rounded-[24px] md:rounded-[40px] shadow-2xl overflow-hidden text-xs border border-slate-100 mx-2 md:mx-0"><div className="p-3 md:p-8 bg-slate-900 text-white font-black uppercase flex justify-between text-[10px] md:text-xs">Suivi Prestataires</div><div className="max-h-[60vh] overflow-y-auto overflow-x-auto custom-scrollbar relative"><table className="w-full text-left min-w-[280px] md:min-w-[700px]"><thead className="bg-slate-50 uppercase text-slate-400 border-b sticky top-0 z-10 shadow-sm text-[6px] md:text-xs tracking-tighter md:tracking-normal"><tr><th className="p-1 md:p-6">Date</th><th className="p-1 md:p-6">Logement</th><th className="p-1 md:p-6">Prestataire</th><th className="p-1 md:p-6 text-right">Montant</th><th className="p-1 md:p-6 text-center">Statut</th></tr></thead><tbody className="divide-y font-bold">{(detailedExpenses || []).map((exp) => (<tr key={exp.id}><td className="p-1 md:p-6 text-[8px] md:text-sm tracking-tighter md:tracking-normal">{formatDateFr(exp.dateRes)}</td><td className="p-1 md:p-6 uppercase text-[8px] md:text-xs">{exp.propertyName}</td><td className="p-1 md:p-6 text-blue-600 uppercase text-[8px] md:text-xs">{exp.person}</td><td className="p-1 md:p-6 text-right text-[8px] md:text-sm">{(exp.amount || 0).toLocaleString('fr-FR')}€</td><td className="p-1 md:p-6 text-center"><span className={`px-1.5 py-0.5 md:px-3 md:py-1 rounded-full text-[6px] md:text-[9px] font-black uppercase ${exp.paymentDate ? 'bg-emerald-100 text-emerald-700' : 'bg-orange-100 text-orange-700'}`}>{exp.paymentDate ? 'Payé' : 'Attente'}</span></td></tr>))}</tbody></table></div></div></div></div>
          <div className="flex-none w-full max-w-full snap-center snap-always px-0 md:px-12 py-6 md:py-12 box-border" style={{ scrollSnapStop: 'always' }}><div className="max-w-7xl mx-auto pb-32 space-y-10"><h2 className="text-3xl font-black uppercase mx-2 md:mx-0">Paramètres</h2><div className="bg-white p-8 rounded-[40px] border-2 border-dashed shadow-xl flex flex-col items-center justify-center text-center mx-2 md:mx-0"><UploadCloud size={40} className="text-blue-600 mb-4"/><h3 className="text-xl font-black uppercase">Importation de Réservations (CSV)</h3><p className="text-xs text-slate-400 mt-2 mb-6">Sélectionnez la plateforme source et collez votre export CSV.</p><div className="flex items-center gap-4 bg-slate-50 p-2 rounded-2xl shadow-inner border border-slate-100"><label className={`flex-1 py-3 px-6 rounded-xl font-black uppercase text-[10px] cursor-pointer transition-all ${importSource === 'Airbnb' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500'}`}><input type="radio" value="Airbnb" checked={importSource === 'Airbnb'} onChange={e => setImportSource(e.target.value)} className="hidden" /> Airbnb</label><label className={`flex-1 py-3 px-6 rounded-xl font-black uppercase text-[10px] cursor-pointer transition-all ${importSource === 'Booking' ? 'bg-blue-900 text-white shadow-lg' : 'text-slate-500'}`}><input type="radio" value="Booking" checked={importSource === 'Booking'} onChange={e => setImportSource(e.target.value)} className="hidden" /> Booking</label></div><textarea value={importText} onChange={(e)=>setImportText(e.target.value)} placeholder={`Collez les lignes ici...`} className="w-full mt-6 p-4 bg-slate-50 border rounded-3xl min-h-[150px] font-mono text-[10px] outline-none" />{importStatus && <p className="mt-4 font-black text-emerald-600 uppercase">{importStatus}</p>}<div className="flex gap-4 w-full mt-8">{reviewList.length === 0 ? (<button onClick={startReview} className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black uppercase shadow-xl hover:bg-blue-600 transition-colors">Analyser le texte</button>) : (<button onClick={confirmImport} disabled={reviewList.filter(r=>r.selected).length === 0} className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black uppercase shadow-xl hover:bg-emerald-600 transition-colors disabled:opacity-50">Importer ({reviewList.filter(r=>r.selected).length})</button>)}</div></div><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-8 mx-2 md:mx-0"><div className="bg-white p-6 rounded-[32px] shadow-lg flex flex-col h-full border-2 border-blue-50"><h3 className="text-[10px] font-black uppercase text-blue-600 mb-4">Logements & iCal</h3><div className="space-y-2 mb-6 flex-1 overflow-y-auto max-h-[400px] text-[10px] font-black uppercase">{(properties || []).map(p=>(<div key={p.id} className="flex flex-col gap-2 p-3 bg-blue-50 rounded-xl"><div className="flex justify-between items-center"><span>{p.name}</span><button onClick={async()=>{if(window.confirm('Supprimer ?'))await deleteDoc(doc(db,'artifacts',appId,'public', 'data', 'properties', p.id))}} className="text-slate-300 hover:text-rose-500"><Trash2 size={14}/></button></div><button onClick={()=>{navigator.clipboard.writeText(generateICalLInk(p.id)); alert('Lien iCal copié !');}} className="w-full bg-white border border-blue-200 text-blue-600 py-2 rounded-lg flex items-center justify-center gap-2 hover:bg-blue-100 transition-colors"><Link size={12}/> Copier lien iCal</button></div>))}</div><form onSubmit={async(e)=>{e.preventDefault(); if(inputProp.name.trim()){await addDoc(collection(db,'artifacts',appId,'public','data','properties'),{name:inputProp.name.trim(),address:inputProp.address.trim()}); setInputProp({name:'',address:''})}}} className="flex flex-col gap-2"><input required value={inputProp.name} onChange={e=>setInputProp({...inputProp,name:e.target.value})} className="p-3 bg-slate-50 rounded-xl text-[10px] outline-none" placeholder="Nom du bien" /><button type="submit" className="bg-slate-900 text-white p-3 rounded-xl font-black text-[10px] uppercase shadow-md">+ Ajouter</button></form></div><div className="bg-white p-6 rounded-[32px] shadow-lg flex flex-col h-full"><h3 className="text-[10px] font-black uppercase text-slate-400 mb-4">Plateformes</h3><div className="space-y-2 mb-6 flex-1 overflow-y-auto max-h-[200px] text-[10px] font-black uppercase">{(availablePlatforms || []).map(p=>(<div key={p} className="flex justify-between items-center p-3 bg-slate-50 rounded-xl"><span>{p}</span><button onClick={()=>{const n = availablePlatforms.filter(x=>x!==p); setAvailablePlatforms(n); updateSettings({platforms:n})}} className="text-slate-300 hover:text-rose-500"><X size={14}/></button></div>))}</div><form onSubmit={(e)=>{e.preventDefault(); if(inputPlat.trim()){const n = [...availablePlatforms, inputPlat.trim()]; setAvailablePlatforms(n); updateSettings({platforms:n}); setInputPlat('')}}} className="flex gap-2"><input value={inputPlat} onChange={e=>setInputPlat(e.target.value)} className="flex-1 p-2 bg-slate-50 border rounded-xl text-[10px]" /><button className="bg-slate-900 text-white p-2 rounded-xl">+</button></form></div><div className="bg-white p-6 rounded-[32px] shadow-lg flex flex-col h-full border-2 border-blue-50"><h3 className="text-[10px] font-black uppercase text-blue-600 mb-4">Prestataires (Emails)</h3><div className="space-y-2 mb-6 flex-1 overflow-y-auto max-h-[200px] text-[10px] font-black uppercase">{(availableProviders || []).map(p=>(<div key={p} className="flex justify-between items-center p-3 bg-slate-50 rounded-xl border border-slate-100"><div><span className="block">{p}</span>{providerEmails[p] && <span className="text-[8px] normal-case text-slate-400 block mt-0.5"><Mail size={8} className="inline mr-1"/>{providerEmails[p]}</span>}</div><button onClick={()=>{const n = availableProviders.filter(x=>x!==p); setAvailableProviders(n); const newEmails = {...providerEmails}; delete newEmails[p]; setProviderEmails(newEmails); updateSettings({providers:n, providerEmails: newEmails});}} className="text-slate-300 hover:text-rose-500"><Trash2 size={14}/></button></div>))}</div><form onSubmit={(e)=>{e.preventDefault(); if(inputProv.trim()){const n = [...availableProviders, inputProv.trim()]; setAvailableProviders(n); const newEmails = {...providerEmails}; if (inputProvEmail.trim()) newEmails[inputProv.trim()] = inputProvEmail.trim(); setProviderEmails(newEmails); updateSettings({providers:n, providerEmails: newEmails}); setInputProv(''); setInputProvEmail('');}}} className="flex flex-col gap-2"><input value={inputProv} onChange={e=>setInputProv(e.target.value)} className="w-full p-2.5 bg-slate-50 border rounded-xl text-[10px]" placeholder="Nom (ex: Dias)" /><input type="email" value={inputProvEmail} onChange={e=>setInputProvEmail(e.target.value)} className="w-full p-2.5 bg-slate-50 border rounded-xl text-[10px]" placeholder="Email (facultatif)" /><button type="submit" className="bg-blue-600 text-white p-2.5 rounded-xl font-black text-[10px] uppercase">+ Ajouter</button></form></div><div className="bg-white p-6 rounded-[32px] shadow-lg flex flex-col h-full"><h3 className="text-[10px] font-black uppercase text-slate-400 mb-4">Services</h3><div className="space-y-2 mb-6 flex-1 overflow-y-auto max-h-[200px] text-[10px] font-black uppercase">{(availableServiceTypes || []).map(p=>(<div key={p} className="flex justify-between items-center p-3 bg-slate-50 rounded-xl"><span>{p}</span><button onClick={()=>{const n = availableServiceTypes.filter(x=>x!==p); setAvailableServiceTypes(n); updateSettings({services:n})}} className="text-slate-300 hover:text-rose-500"><X size={14}/></button></div>))}</div><form onSubmit={(e)=>{e.preventDefault(); if(inputSvc.trim()){const n = [...availableServiceTypes, inputSvc.trim()]; setAvailableServiceTypes(n); updateSettings({services:n}); setInputSvc('')}}} className="flex gap-2"><input value={inputSvc} onChange={e=>setInputSvc(e.target.value)} className="flex-1 p-2 bg-slate-50 border rounded-xl text-[10px]" /><button className="bg-slate-900 text-white p-2 rounded-xl">+</button></form></div></div></div></div>
        </div>
      </main>
