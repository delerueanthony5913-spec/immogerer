...
const appId = typeof __app_id !== 'undefined' ? __app_id : 'immogerer-prod-final';
...
export default function App() {
  ...
  // --- ACTIONS ---
  const updateSettings = async (n) => user && await setDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'settings', 'config'), n, { merge: true });
  ...
  const saveRes = async (e) => {
    ...
    if (editingResId) await setDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'tenants', editingResId), d);
    else await addDoc(collection(db, 'artifacts', appId, 'users', user.uid, 'tenants'), d);
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row font-sans text-slate-900 overflow-hidden">
      ...
      <aside className={`fixed md:sticky top-0 left-0 z-40 w-64 h-full md:h-screen bg-white border-r border-gray-100 flex flex-col transform md:translate-x-0 transition-transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="p-4 border-t"><div className="bg-emerald-50 text-emerald-700 p-3 rounded-xl flex items-center gap-2 text-[9px] font-black uppercase"><div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></div> Cloud Connecté</div></div>
      </aside>
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto h-screen custom-scrollbar">
        <div className="max-w-6xl mx-auto pb-24">
          ...
          {activeTab === 'planning' && (
            <div className="space-y-6 animate-in fade-in">
              <div className="flex flex-wrap items-center justify-between gap-4">
                ...
                <button onClick={() => { setEditingResId(null); setFormData({ propertyId: properties[0]?.id || '', name: '', startDate: '', endDate: '', paymentDate: '', platform: 'Airbnb', isUrssaf: true, displayedAmount: '', cityTax: '', bankFees: '', grossAmount: '', platformFees: '', deposit: '', resExpenses: [] }); setIsModalOpen(true); }} className="bg-blue-600 text-white px-6 py-2.5 rounded-xl font-black text-xs uppercase shadow-lg hover:bg-blue-700 transition-all">+ Nouvelle</button>
              </div>
              <div className="bg-white rounded-[32px] border shadow-sm overflow-hidden text-xs">
                <table className="w-full text-left">
                  <thead className="bg-slate-50 font-black uppercase tracking-widest border-b text-[9px]">
                    <tr><th className="p-5 cursor-pointer hover:text-blue-600" onClick={() => setSortConfig({ key: 'property', direction: sortConfig.direction === 'asc' ? 'desc' : 'asc' })}>Bien</th><th className="p-5 cursor-pointer hover:text-blue-600" onClick={() => setSortConfig({ key: 'name', direction: sortConfig.direction === 'asc' ? 'desc' : 'asc' })}>Client</th><th className="p-5 text-center">Dates</th><th className="p-5 text-right">Net Reçu</th><th className="p-5 text-center">Statut</th></tr>
                  </thead>
          ...
          {activeTab === 'properties' && (
            <div className="space-y-6 animate-in fade-in">
              <div className="flex justify-between items-center"><h2 className="text-xl font-black uppercase tracking-widest text-slate-800">Mes Logements</h2><button onClick={() => setIsPropertyFormOpen(!isPropertyFormOpen)} className="bg-slate-900 text-white px-5 py-2 rounded-xl text-xs font-black uppercase tracking-widest shadow-lg transition-all">+ Ajouter</button></div>
              {isPropertyFormOpen && (
                <form onSubmit={async (e) => { e.preventDefault(); const fd = new FormData(e.target); await addDoc(collection(db, 'artifacts', appId, 'users', user.uid, 'properties'), { name: fd.get('n'), address: fd.get('a'), rent: parseFloat(fd.get('r')) }); setIsPropertyFormOpen(false); }} className="bg-white p-8 rounded-[32px] border-2 border-blue-50 grid grid-cols-1 md:grid-cols-3 gap-6 animate-in slide-in-from-top-4 shadow-xl">
                  <input name="n" required placeholder="Nom (ex: Studio Paris)" className="p-4 bg-slate-50 border rounded-2xl font-bold outline-none" /><input name="a" required placeholder="Adresse" className="p-4 bg-slate-50 border rounded-2xl font-bold outline-none" /><input name="r" type="number" required placeholder="Prix/nuit (€)" className="p-4 bg-slate-50 border rounded-2xl font-bold outline-none" /><button type="submit" className="md:col-span-3 bg-blue-600 text-white p-4 rounded-2xl font-black shadow-lg uppercase text-xs">Enregistrer</button>
                </form>
              )}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {properties.map(p => (
                  <div key={p.id} className="bg-white p-6 rounded-3xl border relative group transition-all hover:border-blue-200 shadow-sm">
                    <div className="flex justify-between items-start mb-4"><div><h4 className="font-black text-slate-800">{p.name}</h4><p className="text-[9px] text-slate-400 font-black uppercase flex items-center gap-1 mt-1"><MapPin size={10} /> {p.address}</p></div><button onClick={async () => await deleteDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'properties', p.id))} className="text-slate-200 hover:text-red-500 transition-colors p-1"><Trash2 size={16} /></button></div>
          ...
          <div className="bg-white rounded-[48px] shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col scale-in-center overflow-hidden">
            ...
            <form onSubmit={saveRes} className="p-10 space-y-8 overflow-y-auto flex-1 custom-scrollbar text-xs">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-1"><label className="text-[9px] font-black uppercase ml-2 text-slate-400">Logement</label><select required value={formData.propertyId} onChange={e => setFormData({ ...formData, propertyId: e.target.value })} className="w-full p-4 bg-slate-50 border rounded-2xl font-bold shadow-inner">{properties.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
              ...
              <div className="flex justify-between items-center pt-8 border-t">
                ...
                <div className="flex gap-4">
                  {editingResId && <button type="button" onClick={() => deleteRes(editingResId)} className="px-6 py-4 text-red-500 font-bold text-[10px] uppercase">Supprimer</button>}
